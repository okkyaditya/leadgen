...
class UpdateMitraRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('updateMitra', $this->route('mitra'));
    }
    ...
    public function rules()
    {
        ...
        $rules = [
            ...
            'nik' => 'required|string|digits:16|regex:/^[0-9]+$/|unique:users,nik,' . $mitra->id,
            'telepon' => 'required|string|max:16|regex:/^[0-9]+$/|unique:users,telepon,' . $mitra->id,
            'email' => 'required|string|email|max:191|unique:users,email,' . $mitra->id,
            'password' => 'nullable|string|min:8',
            'profesi' => 'required|string|max:100',
            'tanggal_lahir' => 'required|date',
            'domisili' => 'required|string|max:150',
            'is_active' => 'required|boolean',
            'is_active_reason' => 'nullable|string|max:255',
        ];
